...
export default function AppRent() {
  ...
  useEffect(()=>{
    ...
    async function fetchSheet(){
      try{
        ...
        if (!aborted) setSheetData(rows);
      }catch(err){
        console.error('Failed fetch sheet', err);
      }finally{ if(!aborted) setLoading(false); }
    }
    fetchSheet();
    const id = setInterval(fetchSheet, REFRESH_SECONDS * 1000);
  ...
  const availability = useMemo(()=>{
    ...
    for (const r of sheetData){
      ...
      const apartment = r.apartment ?? r.Apartment ?? r.apartmentName ?? '';
      ...
      if (map[apartment]) map[apartment][date] = { price: isNaN(price) ? null : price, available };
    }
    return map;
  },[sheetData]);
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      <div className="mb-2">
                        ...
                        <input type="date" value={start} onChange={e=>setStart(e.target.value)} className="mt-1 p-2 border rounded w-full" />
                      </div>
                      <div className="mb-2">
                        <label className="block text-sm">{t.endDate}</label>
                        <input type="date" value={end} onChange={e=>setEnd(e.target.value)} className="mt-1 p-2 border rounded w-full" />
                      </div>
                    ...
                    <div>
                      <h4 className="font-semibold">{lang==='es'?'Calendario & Precios':'Calendar & Prices'}</h4>
                      ...
                      <div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                        {[0,1].map(monthOffset => (
                          <CalendarMonth key={monthOffset} monthOffset={monthOffset} apartmentKey={selectedApt} availability={availability[selectedApt]||{}} lang={lang} />
                        ))}
                      </div>
                    </div>
                  </div>
          ...
          {tab==='contact' && (
            <section className="bg-white p-4 rounded shadow">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div>
                  <h3 className="font-semibold mb-2">{lang==='es'?'Dónde estamos':'Where we are'}</h3>
                  <div className="bg-gray-100 h-48 flex items-center justify-center rounded">Map placeholder (add embed)</div>
                  <p className="text-sm text-gray-600 mt-2">{lang==='es'?'Añade aquí la dirección o un embed de Google Maps.':'Add your address or a Google Maps embed here.'}</p>
                </div>
              </div>
            </section>
          )}
            ...
            <section className="bg-white p-4 rounded shadow">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="bg-gray-100 h-64 flex items-center justify-center rounded">Google Maps embed placeholder</div>
                </div>
              </div>
            </section>
          )}
...
function CalendarMonth({ monthOffset=0, apartmentKey, availability, lang }){
  const now = new Date();
    ...
    <div className="bg-white p-2 rounded shadow-sm">
      ...
      <div className="grid grid-cols-7 gap-1 text-xs">
        ...
        {cells.map((c, i)=>{
          ...
          return (
            <div key={i} className={`h-14 p-1 border rounded text-xs flex flex-col justify-between ${available? 'bg-green-50':'bg-red-50'}`}>
              <div className="text-right">{c.getDate()}</div>
              <div className="text-center">{priceText}</div>
            </div>
          );
        })}
      </div>
